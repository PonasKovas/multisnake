...
impl Bot {
	pub fn start(port: u16, nickname: &str) {
	    ...
	    let my_id = match read_from_stream(&mut stream) {
	        Err(_) => {
	            println!("Bot {} lost connection after requesting to join game", nickname);
	            return;
	        }
	        Ok(bytes) => {
	            if bytes[0] == 0x05 {
	                ...
	                println!("Bot {} received a corrupted message from server: disconnecting.", nickname);
	                return;
	            }
	        }
	    };
	...
	pub fn parse_game_data(self: &mut Self, data: &[u8]) -> (HashMap<(i8, i8), u8>, HashMap<(i8, i8), u16>, bool) {
		// Parse the data
        ...
        for _snake in 0..snake_amount {
            ...
            for _character in 0..u8::from_be_bytes([data[i+2]]) { i += 1; }
            if id == self.my_id {
	...
	pub fn turn(self: &mut Self, data: (HashMap<(i8, i8), u8>, HashMap<(i8, i8), u16>, bool)) {
		...
		'x: for x in -3i8..=3i8 {
			for y in -3i8..=3i8 {
				if data.1.contains_key(&(x, y)) {
					if data.1[&(x, y)] != self.my_id {
		...
		// So basically, if they're different, toggle fast mode.
		if other_snakes_nearby ^ data.2 {
		...
		for (food_pos, amount) in &data.0 {
			let s = (*amount as f32).powf(2.0) / ((food_pos.0 as i16).pow(2) + (food_pos.1 as i16).pow(2)) as f32;
			if s > target.1 {
				target = (*food_pos, s);
			}
		}
		...
		let norm_vec = (
			(target.0).0 as f32/(( ((target.0).0 as i16).pow(2) + ((target.0).1 as i16).pow(2) ) as f32).sqrt(),
			(target.0).1 as f32/(( ((target.0).0 as i16).pow(2) + ((target.0).1 as i16).pow(2) ) as f32).sqrt(),
		);
		let dir = if (norm_vec.0).abs() == (norm_vec.1).abs() {
			...
			(
				if (norm_vec.0).abs() > (norm_vec.1).abs() { (norm_vec.0).round() as i8 } else { 0i8 },
				if (norm_vec.1).abs() > (norm_vec.0).abs() { (norm_vec.1).round() as i8 } else { 0i8 },
			)
		};
